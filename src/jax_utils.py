...
def jax_gradient(f, dx, dy):
    grad_x = jax_dx(f, h=dx)  # Gradient in x-direction
    grad_y = jax_dy(f, h=dy)  # Gradient in y-direction
...
@jit
def smooth_force(force, kernel_size=5, sigma=1.0):
    gauss_kernel_2d = jnp.exp(-(jnp.arange(-(kernel_size//2), (kernel_size//2)+1)**2) / (2 * sigma**2))
    gauss_kernel = jnp.outer(gauss_kernel_2d, gauss_kernel_2d)
    gauss_kernel = gauss_kernel / jnp.sum(gauss_kernel)
    force = jnp.stack([jax.scipy.signal.convolve2d(force[..., 0], gauss_kernel),
                       jax.scipy.signal.convolve2d(force[..., 1], gauss_kernel)], axis=-1)
    force = force[kernel_size//2:-kernel_size//2 + 1, kernel_size//2:-kernel_size//2 + 1]
    return force
...
def jax_curvature(phi, dx, dy):
    ...
    grad_phi = jnp.stack([jax_dx(phi, h=dx), 
                          jax_dy(phi, h=dy)], axis=-1)  # Gradient of phi, shape: (M, N, 2)
    ...
    curvature_value = jax_dx(normalized_grad_phi[..., 0], h=dx) +  \
                      jax_dy(normalized_grad_phi[..., 1], h=dy)
...
def jax_improved_curvature(phi, dx, dy):
    ...
    div_n = jax_dx(n_x, h=dx) + jax_dy(n_y, h=dy)
...
def jax_apply_contact_angle_boundary_conditions(phi, dx, dy, contact_angle=90):
    ...
    grad_phi_x = jax_dx(phi, h=dx)[:, 1]  # x-component of gradient at y=1
    grad_phi_y = jax_dy(phi, h=dy)[:, 1]  # y-component of gradient at y=1
    ...
    phi_new = phi_new.at[:, -1].set(-1.0)  # Top boundary
    phi_new = phi_new.at[0, :].set(-1.0)    # Left boundary
    phi_new = phi_new.at[-1, :].set(-1.0)  # Right boundary
...
def poisson_2d(rhs, dx, dy):
    """Solve Poisson equation using FFT, ensuring real output"""
    Nx, Ny = rhs.shape
    # Compute FFT of the right-hand side
    rhs_hat = jnp.fft.rfft2(rhs)
    
    # Set up wavenumbers
    kx = 2 * jnp.pi * jnp.fft.fftfreq(Nx, dx)
    ky = 2 * jnp.pi * jnp.fft.rfftfreq(Ny, dy)
    kx_grid, ky_grid = jnp.meshgrid(kx, ky, indexing='ij')
    
    # Avoid division by zero at k=0
    k_squared = kx_grid**2 + ky_grid**2
    k_squared = k_squared.at[0, 0].set(1.0)  # Replace zero with 1.0
    
    # Solve in Fourier space
    sol_hat = rhs_hat / (-k_squared)
    sol_hat = sol_hat.at[0, 0].set(0.0)  # Set mean to zero
    
    # Transform back to real space
    sol = jnp.fft.irfft2(sol_hat, s=rhs.shape)
    return sol
