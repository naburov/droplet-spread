...
from jax_utils import jax_dx, jax_dy, jax_calculate_density
...
def create_joint_plot(phi, U, P, surface_tension, dt, step, dx, dy, mass, rho1, rho2, cur_t, save_path=None, plotting_params=None):
    """Create a joint plot with multiple subplots."""
    # Calculate derived fields
    U_magnitude = np.sqrt(U[..., 0]**2 + U[..., 1]**2)
    divergence = jax_dx(U[..., 0], h=dx) + jax_dy(U[..., 1], h=dy)
    ...
    # Setup the figure with configurable parameters
    fig = plt.figure(figsize=plotting_params['figure_size'], dpi=plotting_params['dpi'])
    ...
    im1 = ax1.imshow(phi.T, origin='lower', extent=[0, 1, 0, 1], cmap=plotting_params['colormap'])
    if plotting_params['show_contours']:
        ax1.contour(X, Y, phi.T, levels=[0], colors='r', linewidths=2)
    if plotting_params['show_colorbar']:
        fig.colorbar(im1, ax=ax1, label='Phase Value')
    ax1.set_title('Phase Field', fontsize=plotting_params['title_fontsize'])
    ax1.set_xlabel('X-axis', fontsize=plotting_params['label_fontsize'])
    ax1.set_ylabel('Y-axis', fontsize=plotting_params['label_fontsize'])
    ax1.tick_params(labelsize=plotting_params['tick_fontsize'])
    
    # Plot 2: Surface Tension Force
    ax2 = fig.add_subplot(gs[0, 1])
    im2 = ax2.imshow(ST_magnitude.T, origin='lower', extent=[0, 1, 0, 1], cmap=plotting_params['colormap'])
    if plotting_params['show_contours']:
        ax2.contour(X, Y, phi.T, levels=[0], colors='r', linewidths=2)
    if plotting_params['show_colorbar']:
        fig.colorbar(im2, ax=ax2, label='Force Magnitude')
    ax2.set_title('Surface Tension Force', fontsize=plotting_params['title_fontsize'])
    ax2.set_xlabel('X-axis', fontsize=plotting_params['label_fontsize'])
    ax2.set_ylabel('Y-axis', fontsize=plotting_params['label_fontsize'])
    ax2.tick_params(labelsize=plotting_params['tick_fontsize'])
    
    # Plot 3: Velocity Streamlines
    ax3 = fig.add_subplot(gs[0, 2])
    im3 = ax3.imshow(U_magnitude.T, origin='lower', extent=[0, 1, 0, 1], cmap=plotting_params['colormap'])
    if plotting_params['show_contours']:
        ax3.contour(X, Y, phi.T, levels=[0], colors='r', linewidths=2)
    # Add streamlines
    if plotting_params['show_velocity_vectors']:
        ax3.streamplot(x, y, U[..., 0].T, U[..., 1].T, 
                      density=plotting_params['vector_density']/10, 
                      color='white')
    if plotting_params['show_colorbar']:
        fig.colorbar(im3, ax=ax3, label='Speed')
    ax3.set_title('Velocity Field Streamlines', fontsize=plotting_params['title_fontsize'])
    ax3.set_xlabel('X-axis', fontsize=plotting_params['label_fontsize'])
    ax3.set_ylabel('Y-axis', fontsize=plotting_params['label_fontsize'])
    ax3.tick_params(labelsize=plotting_params['tick_fontsize'])
    
    # Plot 4: Velocity Magnitude
    ax4 = fig.add_subplot(gs[1, 0])
    im4 = ax4.imshow(U_magnitude.T, origin='lower', extent=[0, 1, 0, 1], cmap=plotting_params['colormap'])
    if plotting_params['show_contours']:
        ax4.contour(X, Y, phi.T, levels=[0], colors='r', linewidths=2)
    if plotting_params['show_colorbar']:
        fig.colorbar(im4, ax=ax4, label='Speed')
    ax4.set_title('Velocity Magnitude', fontsize=plotting_params['title_fontsize'])
    ax4.set_xlabel('X-axis', fontsize=plotting_params['label_fontsize'])
    ax4.set_ylabel('Y-axis', fontsize=plotting_params['label_fontsize'])
    ax4.tick_params(labelsize=plotting_params['tick_fontsize'])
    
    # Plot 5: Pressure Field
    ax5 = fig.add_subplot(gs[1, 1])
    im5 = ax5.imshow(P.T, origin='lower', extent=[0, 1, 0, 1], cmap=plotting_params['colormap'])
    if plotting_params['show_contours']:
        ax5.contour(X, Y, phi.T, levels=[0], colors='k', linewidths=2)
    if plotting_params['show_colorbar']:
        fig.colorbar(im5, ax=ax5, label='Pressure')
    ax5.set_title('Pressure Field', fontsize=plotting_params['title_fontsize'])
    ax5.set_xlabel('X-axis', fontsize=plotting_params['label_fontsize'])
    ax5.set_ylabel('Y-axis', fontsize=plotting_params['label_fontsize'])
    ax5.tick_params(labelsize=plotting_params['tick_fontsize'])
    
    # Plot 6: Density Field
    ax6 = fig.add_subplot(gs[1, 2])
    im6 = ax6.imshow(rho.T, origin='lower', extent=[0, 1, 0, 1], cmap=plotting_params['colormap'])
    if plotting_params['show_contours']:
        ax6.contour(X, Y, phi.T, levels=[0], colors='k', linewidths=2)
    if plotting_params['show_colorbar']:
        fig.colorbar(im6, ax=ax6, label='Density')
    ax6.set_title('Density', fontsize=plotting_params['title_fontsize'])
    ax6.set_xlabel('X-axis', fontsize=plotting_params['label_fontsize'])
    ax6.set_ylabel('Y-axis', fontsize=plotting_params['label_fontsize'])
    ax6.tick_params(labelsize=plotting_params['tick_fontsize'])
    
    # Add simulation information
    plt.figtext(0.5, 0.12, f'Time Step: {step}\nSimulation Time: {cur_t:.5f}', ha='center')
